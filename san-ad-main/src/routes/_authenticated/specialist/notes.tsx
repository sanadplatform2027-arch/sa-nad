...
export const Route = createFileRoute("/_authenticated/specialist/notes")({
  component: NotesPage,
});
...
function NotesPage() {
  ...
  const notes = useQuery({
    queryKey: ["specialist", user?.id, "notes", ids],
    queryFn: async () => {
  ...
  const create = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["specialist", user?.id, "notes"] });
    },
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">الملاحظات النفسية والسلوكية</h1>
          <p className="text-sm text-muted-foreground">تسجيل الملاحظات والصعوبات والتحسن</p>
        </div>
