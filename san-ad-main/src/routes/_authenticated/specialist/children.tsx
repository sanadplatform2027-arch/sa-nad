...
export const Route = createFileRoute("/_authenticated/specialist/children")({
  component: SpecialistChildren,
});

function SpecialistChildren() {
  const { user } = useAuth();
  const q = useQuery({
  ...
  return (
    <div className="space-y-6">
      <header>
        <h1 className="text-2xl font-bold">الأطفال المتابعين</h1>
        <p className="text-sm text-muted-foreground">قائمة الحالات التي تتابعها كأخصائي نفسي — تسجيل الأطفال الجدد يتم عبر الإدارة</p>
      </header>

      {q.isLoading ? (
        <div className="flex justify-center py-12"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div>
      ) : (q.data ?? []).length === 0 ? (
        <EmptyState icon={Users} title="لا توجد حالات بعد" description="ستظهر هنا الأطفال الذين تم تخصيصهم لمتابعتك." />
      ) : (
