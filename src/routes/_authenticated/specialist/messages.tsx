...
export const Route = createFileRoute("/_authenticated/specialist/messages")({
...
function MessagesPage() {
  ...
  const notifications = useQuery({
    queryKey: ["specialist", user?.id, "notifications"],
    queryFn: async () => {
  ...
  const messages = useQuery({
    queryKey: ["specialist", user?.id, "messages"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("messages")
  ...
  return (
    <div className="space-y-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground">التواصل مع أولياء الأمور والمعلمين</p>
      </header>
