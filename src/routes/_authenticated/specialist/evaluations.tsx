...
export const Route = createFileRoute("/_authenticated/specialist/evaluations")({
...
function EvaluationsPage() {
  ...
  const grades = useQuery({
    queryKey: ["specialist", user?.id, "grades", ids],
    queryFn: async () => {
  ...
  const create = useMutation({
    ...
    onSuccess: () => {
      ...
      setOpen(false);
      qc.invalidateQueries({ queryKey: ["specialist", user?.id, "grades"] });
    },
    ...
    <div className="space-y-6">
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">تسجيل نتائج التقييمات والنقاط</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button><Plus className="me-2 h-4 w-4" />تقييم جديد</Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader><DialogTitle>تسجيل تقييم</DialogTitle></DialogHeader>
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><Label>المادة / المهارة</Label><Input value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })} /></div>
                <div>
                  <Label>النوع</Label>
      ...
      {grades.isLoading ? (
        ...
        <EmptyState icon={BarChart3} title="لا توجد تقييمات بعد" description="ابدأ بتسجيل أول تقييم." />
      ) : (
